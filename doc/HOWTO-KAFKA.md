...
The remote-host is required and names the address of the broker.  For Kafka it's usually host:port,
...
Meta.Token) and should be specific to the cluster that is reporting.  The server will check that the
token corresponds to the cluster identifier in the topic and in the data.  If the token is used then
there must be TLS.
...
The `incomplete` option, if true, triggers the inclusion of data about pending and running jobs.
...
transmissions for the job in the PENDING state will be seen.  However, transmission of data is
optimized, and redundant data are omitted in subsequent transmissions, and if there are no pertient
data changes - for example, if a new `PENDING` record has the same contents as one already sent
because no settings have changed - then the second record will not be sent at all.  What is deemed
pertient or redundant is up for discussion for both `PENDING` and `RUNNING` jobs.  In any case, the
recipient must be prepared to reconstruct the complete data stream from the initial record for the
`(job_id,job_step)` and any subsequent deltas, applied in order.
