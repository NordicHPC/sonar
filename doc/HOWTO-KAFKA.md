...
In production, we may be able to use the system certificate store.  On linux, the `ca-certificates`
package must be installed on each node for this to work.  The `ca-file` property of the daemon's
.ini file should be set to the value `probe`.
