...
//    - api token within data packet is enough to at least discard bogus data on back-end
//      but not ideal
...
//   clients with that cluster.role combination - like a broadcast.
//
    ...
    fn init(
        ...
        */
...

        {
            ...
            thread::spawn(move || {
                kafka_producer(kafka_receiver, client_id, sender, host, verbose);
            });
        }
