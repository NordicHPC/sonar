...
pub fn get_amd_information(
    ...
    if let Some(concise_raw_text) = command::safe_command(AMD_CONCISE_COMMAND, TIMEOUT_SECONDS) {
        if let Some(showpidgpus_raw_text) = command::safe_command(AMD_SHOWPIDGPUS_COMMAND, TIMEOUT_SECONDS) {
...
fn extract_amd_information(concise_raw_text: &str, showpidgpus_raw_text: &str, user_by_pid: &HashMap<String, String>) -> Vec<nvidia::Process> {
	...
	devs.iter().for_each(|dev| { num_processes_per_device[*dev] += 1 })
    });
	...
	devs.iter().for_each(|dev| {
		...
		device: *dev as i32,
		pid: pid.to_string(),
		user: if let Some(u) = user_by_pid.get(&pid.to_string()) { u.to_string() } else { "_zombie_".to_owned() + pid },
...
pub fn parse_concise_command<'a>(raw_text: &'a str) -> Vec<(f64, f64)> {
	...
	if hdr[hdr.len()-2] == "VRAM%" && hdr[hdr.len()-1] == "GPU%" {
		...
		let mem = fields[fields.len()-2].strip_suffix('%').unwrap().parse::<f64>().unwrap_or_default();
		let gpu = fields[fields.len()-1].strip_suffix('%').unwrap().parse::<f64>().unwrap_or_default();
