...
for(let swname in parsed_data.software) {
	let swobj = parsed_data.software[swname];
	histo_dataset.push({name: swname, percent: swobj.percent});

	let subcall_list = [];
	for(let subcall in swobj.subcalls) {
		subcall_list.push({name: subcall, percent: swobj.subcalls[subcall]});
	}
