...
type SampleDisk struct {
	...
	//   appears new fields are always appended.
	// - Counters may sometimes wrap around, and they will be reset on reboot.  See also the Boot
	//   field in SampleSystem.
	// - Field interpretation may also vary between versions.
	// - The field should be considered experimental; when it was introduced, we did not know which
	//   data would be interesting.  It may be augmented by other fields in the future.
	//
	// [1] https://github.com/torvalds/linux/blob/master/Documentation/admin-guide/iostats.rst.
	// [2] https://elixir.bootlin.com/linux/v4.18.20/source/Documentation/iostats.txt.
