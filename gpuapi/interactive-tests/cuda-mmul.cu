...
// This performs a tiled mmul in Cuda.  (Culled from an old test case.)
//
// To compile this, load the NVIDIA tool chain (eg, `module load gcccuda` on the UiO ML nodes) then
// `make cuda-mmul`.
//
...
//
// A single iteration takes about 15s on an RTX 2080 Ti.
...
static void copy_to_device(void* dev_dest, void* host_src, size_t nbytes) {
  cudaError_t err;
  if ((err = cudaMemcpy(dev_dest, host_src, nbytes, cudaMemcpyHostToDevice)) != 0) {
...
int main(int argc, char** argv) {
  ...
  // TODO: It would seem that the copying to the device memory could be done by
  // cuda in parallel if the source arrays are cuda host memory blocks.
  ...
  for ( int it=0 ; it < iterations ; it++ ) {
    ...
    cudaDeviceSynchronize();	// Or the timing is all wrong
  }
