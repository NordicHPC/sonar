...
static int check_result(int result, char desc[128]) {
    switch(result) {
        case 1:
            fprintf(stderr, "WARNING: %s - failed - invalid argument\n", desc);
            break;;
        case 2:
...
int amdml_device_get_card_state(uint32_t device, struct amdml_card_state_t* infobuf) {
    ...
    int ret = xrsmi_dev_current_socket_power_get(device, &power);
    if(check_result(ret, "dev_current_socket_power_get") == 0) {
        ...
        if (check_result(ret, "dev_power_get") == 0) {
            infobuf->power = (unsigned)(power / 1000);
        }
    }

    ret = xrsmi_dev_power_cap_get(device, 0, &power);
    if (check_result(ret, "dev_power_cap") == 0) {
